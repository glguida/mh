...
void vtscr_wflush(void);
WIN *vtscr_wopen(int x1, int y1, int x2, int y2, int border,
           int attr, int fg, int bg, int direct, int hl, int rel);
void vtscr_wclose(WIN *win, int replace);
void vtscr_wleave(void);
void vtscr_wreturn(void);
void vtscr_wresize(WIN *w, int x, int y);
void vtscr_wredraw(WIN *w, int newdirect);
void vtscr_wscroll(WIN *win, int dir);
void vtscr_wlocate(WIN *win, int x, int y);
void vtscr_wputc(WIN *win, wchar_t c);
void vtscr_wdrawelm(WIN *win, int y, ELM *e);
void vtscr_wputs(WIN *win, const char *s);
int vtscr_wprintf(WIN *, const char *, ...)
        __attribute__((format(printf, 2, 3)));
void vtscr_wbell(void);
void vtscr_wcursor(WIN *win, int type);
void vtscr_wtitle(WIN *w, int pos , const char *s);
void vtscr_wcurbar(WIN *w, int y , int attr);
int vtscr_wselect(int x, int y, const char *const *choices,
            void (*const *funlist)(void) ,
            const char *title , int attr , int fg , int bg );
void vtscr_wclrch(WIN *w, int n);
void vtscr_wclrel(WIN *w);
void vtscr_wclreol(WIN *w);
void vtscr_wclrbol(WIN *w);
void vtscr_wclreos(WIN *w);
void vtscr_wclrbos(WIN *w);
void vtscr_winclr(WIN *w);
void vtscr_winsline(WIN *w);
void vtscr_wdelline(WIN *w);
void vtscr_winschar(WIN *w);
void vtscr_winschar2(WIN *w, wchar_t c, int move);
void vtscr_wdelchar(WIN *w);
int  vtscr_wgets(WIN *win, char *s, int linemax, int totmax);
int  vtscr_wgetwcs(WIN *win, wchar_t *s, int linemax, int totmax);
void win_end(void);
#ifdef BBS
...
void vtscr_wdrawelm_inverse( WIN *w, int y, ELM *e);
void vtscr_wdrawelm_var(WIN *w, ELM *e, wchar_t *buf);

/*
 * Some macro's that can be used as functions.
 */
#define vtscr_wsetregion(w, z1, z2) (((w)->sy1=(w)->y1+(z1)),((w)->sy2=(w)->y1+(z2)))
#define vtscr_wresetregion(w) ( (w)->sy1 = (w)->y1, (w)->sy2 = (w)->y2 )
#define vtscr_wgetattr(w) ( (w)->attr )
#define vtscr_wsetattr(w, a) ( (w)->attr = (a) )
#define vtscr_wsetfgcol(w, fg) ( (w)->color = ((w)->color & 15) + ((fg) << 4))
#define vtscr_wsetbgcol(w, bg) ( (w)->color = ((w)->color & 240) + (bg) )
#define vtscr_wsetam(w) ( (w)->wrap = 1 )
#define vtscr_wresetam(w) ( (w)->wrap = 0 )
