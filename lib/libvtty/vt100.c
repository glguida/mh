...
#ifndef ICONV_NOT_DISABLED
#define using_iconv() 0
#endif
...
void vt_pinit(WIN *win, int fg, int bg)
{
  ...
  newy1 = 0;
  newy2 = vt_win->ys - 1;
  vtty_wresetregion(vt_win);
  if (fg > 0)
    vt_fg = fg;
  if (bg > 0)
    vt_bg = bg;
  vtty_wsetfgcol(vt_win, vt_fg);
  vtty_wsetbgcol(vt_win, vt_bg);
}
...
void vt_set(int addlf, int wrap, int bscode,
  ...
  switch(c) {
    ...
    case 'M': /* Cursor up */
      ...
      if (c == 'D') { /* Down. */
        y = vt_win->cury + 1;
        if (y == newy2 + 1)
          vtty_wscroll(vt_win, S_UP);
        else if (vt_win->cury < vt_win->ys)
          vtty_wlocate(vt_win, x, y);
      }
      if (c == 'M')  { /* Up. */
        y = vt_win->cury - 1;
        if (y == newy1 - 1)
          vtty_wscroll(vt_win, S_DOWN);
        else if (y >= 0)
          vtty_wlocate(vt_win, x, y);
      }
      break;
...
#endif
      vt_win->color = savecol; /* HACK should use vtty_wsetfgcol etc */
      vtty_wsetattr(vt_win, saveattr);
      vtty_wlocate(vt_win, savex, savey);
      break;
...
static void state2(int c)
{
  ...
  switch (c) {
    ...
    case 'J': /* Screen erasing */
      ...
      y = vt_win->attr;
      if (vt_type == ANSI) {
        vtty_wsetattr(vt_win, XA_NORMAL);
        vtty_wsetfgcol(vt_win, WHITE);
        vtty_wsetbgcol(vt_win, BLACK);
      }
      switch (escparms[0]) {
        case 0:
          vtty_wclreos(vt_win);
    ...
    case 'm': /* Set attributes */
      attr = vtty_wgetattr((vt_win));
      for (f = 0; f <= ptr; f++) {
        if (escparms[f] >= 30 && escparms[f] <= 37)
          vtty_wsetfgcol(vt_win, escparms[f] - 30);
        if (escparms[f] >= 40 && escparms[f] <= 47)
          vtty_wsetbgcol(vt_win, escparms[f] - 40);
        switch (escparms[f]) {
          case 0:
            attr = XA_NORMAL;
            vtty_wsetfgcol(vt_win, vt_fg);
      ...
      vtty_wsetattr(vt_win, attr);
      break;
    case 'L': /* Insert lines */
      if ((x = escparms[0]) == 0)
        x = 1;
      for (f = 0; f < x; f++)
        vtty_winsline(vt_win);
      break;
    case 'M': /* Delete lines */
      if ((x = escparms[0]) == 0)
        x = 1;
      for (f = 0; f < x; f++)
        vtty_wdelline(vt_win);
      break;
    case 'P': /* Delete Characters */
      if ((x = escparms[0]) == 0)
        x = 1;
      for (f = 0; f < x; f++)
        vtty_wdelchar(vt_win);
      break;
    case '@': /* Insert Characters */
      if ((x = escparms[0]) == 0)
        x = 1;
      for (f = 0; f < x; f++)
        vtty_winschar(vt_win);
      break;
    case 'r': /* Set scroll region */
      ...
      vtty_wsetregion(vt_win, newy1, newy2);
      vtty_wlocate(vt_win, 0, newy1);
      break;
    case 'i': /* Printing */
  ...
  for (i = 0; i <= ptr; i++) {
    switch (escparms[i]) {
      ...
      case 25: /* Cursor on/off */
        vtty_wcursor(vt_win, on_off ? CNORMAL : CNONE);
        break;
  ...
  switch (c) {
    case '8':
      ...
      for (y = 0; y < vt_win->ys; y++) {
        vtty_wlocate(vt_win, 0, y);
        for (x = 0; x < vt_win->xs; x++)
          vtty_wputc(vt_win, 'E');
      }
      vtty_wlocate(vt_win, 0, 0);
      vt_win->doscroll = 1;
      vtty_wredraw(vt_win, 1);
      break;
    default:
      /* IGNORED */
      break;
...
static void state7(int c)
{
  ...
  if (state == 1) {
    ...
    if (!strcmp(buf, "cursor.on"))
      vtty_wcursor(vt_win, CNORMAL);
    if (!strcmp(buf, "cursor.off"))
      vtty_wcursor(vt_win, CNONE);
...
void vt_out(int ch)
{
  ...
  switch (c) {
    ...
    case '\t': /* Non - destructive TAB */
      ...
      if (f >= vt_win->xs)
        f = vt_win->xs - 1;
      vtty_wlocate(vt_win, f, vt_win->cury);
      break;
    case 013: /* Old Minix: CTRL-K = up */
      vtty_wlocate(vt_win, vt_win->curx, vt_win->cury - 1);
      break;
  ...
  switch (esc_s) {
    case 0: /* Normal character */
      ...
      if (!using_iconv()) {
        ...
        if (vt_insert)
          vtty_winschar2(vt_win, wc, 1);
        else
          vtty_wputc(vt_win, wc);
      } else {
        vtty_wputc(vt_win, c);
      }
#else
