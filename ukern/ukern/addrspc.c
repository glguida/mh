...
static int __vrange_cmp_key(void *ctx, const void *n, const void *key)
{
...
static int __vrange_cmp_nodes(void *ctx, const void *n1, const void *n2)
{
...
struct pager *addrspc_get_pager(struct addrspc *as, vaddr_t va)
{
	struct vrange *vr;

	spinlock(&as->lock);
	vr = (struct vrange *) rb_tree_find_node(&as->vranges,
						 (void *) &va);
	spinunlock(&as->lock);
	if (vr == NULL)
		return NULL;
