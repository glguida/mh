...
static void
zone_detachentry(struct zone *z, struct __ZENTRY *ze)
{
    uint32_t msb;
...
static void
zone_attachentry(struct zone *z, struct __ZENTRY *ze)
{
    uint32_t msb;
    ...
    dbgprintf("LIST_INSERT(%p + %d, %p), bmap (%lx ->", z->zlist, msb, ze, z->bmap);
    z->bmap |= (1UL << msb);
    dbgprintf(" %lx\n", z->bmap);
...
static struct __ZENTRY *
zone_findfree(struct zone *zn, size_t size)
{
    uint32_t tmp;
    unsigned int minbit;
...
void
ALLOCFUNC(free)(struct zone *z, zaddr_t zaddr, size_t size)
{
...
zaddr_t
ALLOCFUNC(alloc)(struct zone *z, size_t size)
{
    struct __ZENTRY *ze;
    zaddr_t addr = 0;
    long diff;
